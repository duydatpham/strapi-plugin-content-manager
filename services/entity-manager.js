...
module.exports = {
  ...
  async findOneWithCreatorRoles(id, model, populate) {
    let findList = await this.findWithRelationCounts({ _where: [{ id }] }, model, populate);
    let entity = !!findList && !!findList.results && findList.results.length > 0 ? findList.results[0] : null
