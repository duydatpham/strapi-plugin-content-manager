...
function SelectWrapper({
  ...
  const valueCount = get(modifiedData, `___${name}_count`, 0);
  const value = isSingle ? get(modifiedData, name, null) : get(modifiedData, name, [])
  const valueLoaded = isSingle || get(modifiedData, `___${name}_loaded`, false)
  ...
  const getCurrentValue = useCallback(
    async signal => {
      ...
      try {
        ...
        addUpdateKeys({ target: { name: `initialData.___${name}_loaded`, value: true } });
        addUpdateKeys({ target: { name: `initialData.${name}`, value: fromJS(data) } });
      } catch (err) {
        // Silent
      }
    },
  ...
  const associationsLength = isArray(value) ? (value.length || valueCount) : 0;
  ...
  return (
    <Padded>
      ...
      <Padded top size="sm">
        ...
        <Component
          addRelation={value => {
            addRelation({ target: { name: `${name}`, value } });
          }}
          components={{ ClearIndicator, DropdownIndicator, IndicatorSeparator, Option }}
          displayNavigationLink={shouldDisplayRelationLink}
          id={name}
          isDisabled={isDisabled || !valueLoaded}
          isLoading={isLoading || !valueLoaded}
          isClearable
          mainField={mainField}
          move={moveRelation}
          name={name}
